...
function runTheRegister() {
    ...
    try { // Step 4: Add try block
        // Step 4: Test if changeValue is negative and throw an exception
        if (changeValue < 0) {
    ...
    } catch (error) { // Step 4: Add catch block
        // Step 4: Set the innerHTML of the warning element to the exception value
...
function calcChange(changeValue) {
    // Correcting for potential floating-point precision issues by using toFixed(2)
    ...
    changeValue = parseFloat(changeValue.toFixed(2)); // Rounding after subtraction
	...
	changeValue -= 	bill5Amt*5; // Logic error fix: Was bill5Amt * 3, should be * 5
    changeValue = parseFloat(changeValue.toFixed(2)); // Rounding after subtraction
